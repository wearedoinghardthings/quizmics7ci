...
from database import (
    ...
    submit_session, get_results, session_already_completed, get_stats, get_question_stats,
)
...
# ── Formatage texte riche des questions ──────────────────────────────────────
def _fmt(text: str) -> str:
    """
    **texte**              → gras
    *texte*                → italique
    [rouge]texte[/rouge]   → rouge
    [bleu]texte[/bleu]     → bleu
    [vert]texte[/vert]     → vert
    [orange]texte[/orange] → orange
    ...
    t = _re.sub(r"\[rouge\](.+?)\[/rouge\]",    r'<span style="color:#DC2626;font-weight:700">\1</span>', t)
    t = _re.sub(r"\[bleu\](.+?)\[/bleu\]",      r'<span style="color:#2563EB;font-weight:700">\1</span>', t)
    t = _re.sub(r"\[vert\](.+?)\[/vert\]",      r'<span style="color:#059669;font-weight:700">\1</span>', t)
    t = _re.sub(r"\[orange\](.+?)\[/orange\]",  r'<span style="color:#D97706;font-weight:700">\1</span>', t)
    t = t.replace("\n", "<br>")
    return t
...
def _cached_quiz_by_code(code):
    return get_quiz_by_code(code)

@st.cache_data(ttl=60, show_spinner=False)
def _cached_questions(quiz_id):
    return get_questions(quiz_id, shuffled=False)

def _start_quiz(agent, code):
    if not (code or "").strip(): st.warning("Saisissez un code."); return
    quiz = _cached_quiz_by_code(code.strip())
    if not quiz: st.error("Code incorrect ou quiz désactivé."); return
    import random as _rnd
    qs = list(_cached_questions(quiz["id"]))
    if quiz.get("randomize_questions"): _rnd.shuffle(qs)
...
def _tab_question_stats():
    ...
    opts = {q["id"]: f"{q['titre']}  ({q['code']})" for q in quizzes}
    st.markdown('<div style="padding:0 16px">', unsafe_allow_html=True)
    sel_id = st.selectbox("Choisir un quiz", list(opts.keys()),
    ...
    pad()
    stats = get_question_stats(sel_id)
    if not stats:
        st.markdown('<div style="padding:0 16px">', unsafe_allow_html=True)
        st.info("Aucune donnée disponible pour ce quiz — personne ne l'a encore soumis.")
        st.markdown('</div>', unsafe_allow_html=True)
        return

    # KPI global
    total_rep = stats[0]["total"] if stats else 0
    avg_err = round(sum(s["taux_erreur"] for s in stats) / len(stats), 1) if stats else 0
    plus_dur = stats[0] if stats else None  # trié par taux_erreur desc

    st.markdown(f"""
    <div class="kpi-grid" style="grid-template-columns:repeat(3,1fr)">
      <div class="kpi kpi-b"><span class="kpi-ico">📝</span><p class="kpi-val">{len(stats)}</p><p class="kpi-lbl">Questions</p></div>
      <div class="kpi kpi-o"><span class="kpi-ico">❌</span><p class="kpi-val">{avg_err}%</p><p class="kpi-lbl">Erreur moy.</p></div>
      <div class="kpi kpi-r" style="background:linear-gradient(135deg,#FEE2E2,#fff);border:1.5px solid #FCA5A5"><span class="kpi-ico">🎯</span><p class="kpi-val" style="color:#DC2626">{plus_dur['taux_erreur']}%</p><p class="kpi-lbl">+ difficile</p></div>
    </div>""", unsafe_allow_html=True)

    section("Classement des questions par taux d'erreur (du plus difficile au plus facile)")
    ...
    for s in stats:
        ...
        <div style="background:var(--surface);border:1.5px solid var(--border);border-left:4px solid {color};
          ...
          <div style="display:flex;gap:12px;margin-top:6px;font-size:.75rem;color:var(--text3)">
            ...
            <span>💎 {s['points']} pt(s)</span>
          </div>
        </div>"""
    st.markdown(f'<div style="padding:0 16px">{bars_html}</div>', unsafe_allow_html=True)
    ...
    df_stats = pd.DataFrame([{
        "N°":             s["num"],
        "Question":       s["texte"][:80],
        "Type":           TYPES.get(s["type"], s["type"]),
        "Points":         s["points"],
        "Réponses total": s["total"],
        "Correctes":      s["correct"],
        "Erreurs":        s["erreurs"],
        "Taux erreur (%)":s["taux_erreur"],
    } for s in stats])

    quiz_info = get_quiz(sel_id)
    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        df_stats.to_excel(w, index=False, sheet_name="Stats questions")
        ws = w.sheets["Stats questions"]
        # Largeurs colonnes
        for col in ws.columns:
            ml = max((len(str(c.value)) for c in col if c.value), default=10)
            ws.column_dimensions[col[0].column_letter].width = min(ml + 4, 60)
        # Couleur rouge sur les lignes avec taux > 50%
        from openpyxl.styles import PatternFill, Font
        red_fill   = PatternFill("solid", fgColor="FEE2E2")
        orange_fill = PatternFill("solid", fgColor="FEF3C7")
        for row in ws.iter_rows(min_row=2):
            pct_cell = row[7]  # colonne "Taux erreur"
            if pct_cell.value and pct_cell.value >= 60:
                for cell in row: cell.fill = red_fill
            elif pct_cell.value and pct_cell.value >= 30:
                for cell in row: cell.fill = orange_fill
    buf.seek(0)

    fname = f"stats_questions_{quiz_info['code']}_{datetime.now().strftime('%Y%m%d')}.xlsx"
    st.markdown('<div style="padding:0 16px">', unsafe_allow_html=True)
    st.download_button(
        "📥  Exporter les stats questions (Excel)",
        data=buf.getvalue(),
        file_name=fname,
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
...
def render_admin_quiz_edit():
    ...
    if questions:
        for i, q in enumerate(questions):
            with st.expander(f"Q{i+1}  ·  {_TYPES[q['type']]}  ·  {q['texte'][:55]}"):
                if q["type"] in ("single","multiple"):
    ...
    with st.form("faq", clear_on_submit=True):
        st.markdown("""
<div style="background:#F0F4FF;border:1.5px solid #BFDBFE;border-radius:8px;padding:10px 14px;margin-bottom:8px;font-size:.8rem;color:#1E3A8A">
<b>Formatage disponible :</b><br>
<code>**texte**</code> → <b>gras</b> &nbsp;·&nbsp;
<code>*texte*</code> → <i>italique</i> &nbsp;·&nbsp;
<code>[rouge]texte[/rouge]</code> → <span style="color:#DC2626;font-weight:700">rouge</span> &nbsp;·&nbsp;
<code>[bleu]texte[/bleu]</code> → <span style="color:#2563EB;font-weight:700">bleu</span> &nbsp;·&nbsp;
<code>[vert]texte[/vert]</code> → <span style="color:#059669;font-weight:700">vert</span> &nbsp;·&nbsp;
<code>[orange]texte[/orange]</code> → <span style="color:#D97706;font-weight:700">orange</span><br>
Appuyez sur <b>Entrée</b> pour aller à la ligne.
</div>""", unsafe_allow_html=True)
        q_txt = st.text_area("Question *", height=110, placeholder="Ex : Quelle est la **définition** de [rouge]l\'enquête MICS[/rouge] ?")
