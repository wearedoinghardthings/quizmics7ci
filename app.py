...
SUPA_URL_ST = "https://yptijbomgldhwftuicno.supabase.co"
SUPA_KEY_ST = "sb_publishable_FERrIaP0u1ITsyqC9wiB8A_Lr9oOrpv"

def _upload_image(file_obj, label):
    ext  = (file_obj.name.rsplit(".",1)[-1] or "jpg").lower()
    name = f"q_{label}_{int(time.time())}.{ext}"
    r = requests.put(
        f"{SUPA_URL_ST}/storage/v1/object/quiz-images/{name}",
        data=file_obj.getvalue(),
        headers={"apikey": SUPA_KEY_ST, "Authorization": f"Bearer {SUPA_KEY_ST}",
                 "Content-Type": f"image/{ext}", "x-upsert": "true"},
        timeout=30)
    if r.status_code not in (200,201):
        raise Exception(f"Erreur {r.status_code}: {r.text[:100]}")
    return f"{SUPA_URL_ST}/storage/v1/object/public/quiz-images/{name}"
...
def render_agent_quiz():
    ...
    for i,q in enumerate(questions):
        ...
        st.markdown(
            ...
            + (f'<img src="{q["image_url"]}" style="width:100%;max-height:260px;object-fit:contain;border-radius:8px;margin-bottom:8px">' if q.get('image_url') else '')
...
def render_admin_quiz_edit():
    ...
    # ── Reste du formulaire ──
    img_file = st.file_uploader("🖼️ Image (optionnelle)", type=["jpg","jpeg","png","gif","webp"], key="q_img_upload")
    if img_file:
        ...
        if submitted:
            ...
            if err:
                ...
                if img_file is not None:
                    try: img_url = _upload_image(img_file, f"{cur_id}_{ordre}")
                    except Exception as e: st.warning(f"Image non uploadée : {e}")
                if q_type in ("single", "multiple"):
